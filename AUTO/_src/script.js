...
var exp_animation = function(){
	...
	tweenDelay += 1;
	if ( isAutoExpandSequence ) {
		isAutoExpandSequence = false;
		tweenDelay += 1;
		td( tweenDelay, collapseAutoExpand );
	} else {
...
function collapseFinishHandler() { 
	...
	TweenLite.killDelayedCallsTo([fadeInText, collapseAutoExpand]);
...
function init(){
	...
	add_listeners();
	td( 0.3, function(){ Enabler.requestExpand(); });
	
}

// --------------------------------- POLITE ASSET LOADING SCRIPT --------------------------------- //
var images_count = 0;
